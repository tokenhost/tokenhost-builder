...
function resolveNetlifyUploadBuildConfig(schema: ThsSchema): NetlifyUploadBuildConfig | null {
  const deploy = schema.app.deploy?.netlify?.uploads;
  if (!deploy) return null;
  return {
    provider: 'filecoin_onchain_cloud',
    runner: 'background-function',
    privateKeyEnv: String(deploy.privateKeyEnv ?? 'TH_UPLOAD_FOC_PRIVATE_KEY').trim() || 'TH_UPLOAD_FOC_PRIVATE_KEY',
    blobsStore: String(deploy.blobsStore ?? 'tokenhost-upload-jobs').trim() || 'tokenhost-upload-jobs',
    functionsDirectory: normalizeFunctionsDirectory(deploy.functionsDirectory),
    endpointUrl: '/__tokenhost/upload',
    statusUrl: '/__tokenhost/upload-status',
    startFunctionName: 'tokenhost-upload-start',
    statusFunctionName: 'tokenhost-upload-status',
    workerFunctionName: 'tokenhost-upload-worker-background',
    accept: String(process.env.TH_UPLOAD_ACCEPT ?? 'image/png,image/jpeg,image/gif,image/webp,image/svg+xml')
      .split(',')
      .map((x) => x.trim())
      .filter(Boolean),
    maxBytes: parsePositiveIntEnv(process.env.TH_UPLOAD_MAX_BYTES, 10 * 1024 * 1024)
  };
}

function renderNetlifyPackageJson(schema: ThsSchema): string {
  return JSON.stringify(
    {
      name: `${schema.app.slug}-netlify`,
      private: true,
      type: 'module',
      dependencies: {
        '@netlify/blobs': '^8.1.0'
...
function renderNetlifyToml(config: NetlifyUploadBuildConfig): string {
  return `# Generated by Token Host
[build]
  publish = "ui-site"

[functions]
  directory = "${config.functionsDirectory}"
  node_bundler = "esbuild"
  external_node_modules = ["@netlify/blobs"]

[[redirects]]
  from = "${config.endpointUrl}"
  to = "/.netlify/functions/${config.startFunctionName}"
  status = 200
  force = true

[[redirects]]
  from = "${config.statusUrl}"
  to = "/.netlify/functions/${config.statusFunctionName}"
  status = 200
  force = true
`;
}

function renderNetlifyUploadShared(config: NetlifyUploadBuildConfig): string {
...
export const UPLOAD_PROVIDER = ${JSON.stringify(config.provider)};
export const UPLOAD_RUNNER_MODE = 'netlify-background';
export const BLOBS_STORE_NAME = ${JSON.stringify(config.blobsStore)};
export const PRIVATE_KEY_ENV = ${JSON.stringify(config.privateKeyEnv)};
export const STATUS_BASE_PATH = ${JSON.stringify(config.statusUrl)};
export const MAX_BYTES = ${config.maxBytes};
export const DEFAULT_FOC_COMMAND = 'npx -y foc-cli';
export const DEFAULT_CHAIN_ID = 314159;
export const DEFAULT_COPIES = 1;

export function getUploadStore() {
  return getStore(BLOBS_STORE_NAME);
}
...
export function jobRequestKey(jobId) {
...
export function normalizeUploadResult(parsed) {
  ...
  return {
    ...
    runnerMode: UPLOAD_RUNNER_MODE,
    contentType: result?.mimeType ? String(result.mimeType) : null,
    metadata: {
...
export async function setJobStatus(jobId, value) {
  const store = getUploadStore();
  await store.setJSON(jobStatusKey(jobId), value);
}

export async function getJobStatus(jobId) {
  const store = getUploadStore();
  return (await store.get(jobStatusKey(jobId), { type: 'json' })) ?? null;
}

export async function getJobRequest(jobId) {
  const store = getUploadStore();
  return (await store.get(jobRequestKey(jobId), { type: 'json' })) ?? null;
}

export async function setJobRequest(jobId, value) {
  const store = getUploadStore();
  await store.setJSON(jobRequestKey(jobId), value);
}

export async function deleteJobRequest(jobId) {
  const store = getUploadStore();
  await store.delete(jobRequestKey(jobId));
}
...
export async function runShellCommand(command, env) {
  return await new Promise((resolve, reject) => {
    const child = spawn('/bin/bash', ['-lc', command], { env });
    let stdout = '';
    let stderr = '';
...
export async function runFocUploadFromJob(job) {
  const privateKey = String(process.env[PRIVATE_KEY_ENV] ?? '').trim();
  if (!privateKey) {
    throw new Error(\`Missing required Netlify env var \${PRIVATE_KEY_ENV}.\`);
  }
  ...
  try {
    ...
    try {
      ...
      throw new Error('foc-cli did not return valid JSON.');
    }
...
function renderNetlifyUploadStart(config: NetlifyUploadBuildConfig): string {
  return `import {
  MAX_BYTES,
  STATUS_BASE_PATH,
  UPLOAD_PROVIDER,
  ...
  setJobRequest,
  setJobStatus
} from './_tokenhost-upload-shared.mjs';
...
export default async (request) => {
  ...
  const fileName = String(request.headers.get('x-tokenhost-upload-filename') || 'upload.bin').trim() || 'upload.bin';
  const contentType = String(request.headers.get('content-type') || 'application/octet-stream').trim() || 'application/octet-stream';
  const size = body.length;
  await setJobRequest(jobId, {
    jobId,
    fileName,
    contentType,
    size,
  ...
  const workerUrl = new URL('/.netlify/functions/${config.workerFunctionName}', request.url).toString();
  ...
  if (!workerRes.ok && workerRes.status !== 202) {
    await setJobStatus(jobId, {
      ...
      pending: false,
      jobId,
      error: \`Unable to enqueue Netlify background upload (HTTP \${workerRes.status}).\`,
      provider: UPLOAD_PROVIDER,
      runnerMode: UPLOAD_RUNNER_MODE,
      updatedAt: new Date().toISOString()
    });
    return Response.json({ ok: false, error: 'Unable to enqueue Netlify background upload.' }, { status: 500 });
  }
  ...
  return Response.json(
    {
      ...
      pending: true,
      jobId,
      statusUrl: \`\${STATUS_BASE_PATH}?jobId=\${encodeURIComponent(jobId)}\`
    },
    { status: 202 }
  );
};
...
function renderNetlifyUploadStatus(): string {
  return `import { getJobStatus } from './_tokenhost-upload-shared.mjs';

export default async (request) => {
  const url = new URL(request.url);
  const jobId = String(url.searchParams.get('jobId') || '').trim();
  if (!jobId) {
    return Response.json({ ok: false, error: 'Missing jobId query parameter.' }, { status: 400 });
  }

  const status = await getJobStatus(jobId);
  if (!status) {
    return Response.json({ ok: false, error: 'Unknown upload job.' }, { status: 404 });
  }
...
function renderNetlifyUploadWorker(): string {
  return `import { deleteJobRequest, getJobRequest, runFocUploadFromJob, setJobStatus, UPLOAD_PROVIDER, UPLOAD_RUNNER_MODE } from './_tokenhost-upload-shared.mjs';
...
export default async (request) => {
  ...
  if (!job) {
    await setJobStatus(jobId, {
      ...
      pending: false,
      jobId,
      error: 'Upload payload was not found in Netlify Blobs.',
      provider: UPLOAD_PROVIDER,
      runnerMode: UPLOAD_RUNNER_MODE,
      updatedAt: new Date().toISOString()
    });
    return Response.json({ ok: true, pending: false }, { status: 202 });
  }

  try {
    const uploaded = await runFocUploadFromJob(job);
    await setJobStatus(jobId, {
      ok: true,
      pending: false,
      jobId,
...
function renderNetlifyReadme(config: NetlifyUploadBuildConfig): string {
  return `# Netlify Upload Runner

This build output includes Netlify Functions scaffolding for Token Host uploads.

Required Netlify runtime environment variables:
- \`${config.privateKeyEnv}\` - private key used for \`foc-cli upload\`
- \`TH_UPLOAD_FOC_CHAIN\` - optional, defaults to \`314159\`
- \`TH_UPLOAD_FOC_COPIES\` - optional, defaults to \`1\`
- \`TH_UPLOAD_FOC_COMMAND\` - optional, defaults to \`npx -y foc-cli\`
- \`TH_UPLOAD_FOC_WITH_CDN\` - optional
- \`TH_UPLOAD_FOC_DEBUG\` - optional

Generated routes:
- \`${config.endpointUrl}\` -> upload start
- \`${config.statusUrl}\` -> upload status

Implementation notes:
- The start function stores the request body in Netlify Blobs and returns \`202\` with a job id.
- The background worker runs \`foc-cli\` and writes the final result back to Netlify Blobs.
- The browser polls the status endpoint until the upload completes.
`;
}

function writeNetlifyUploadArtifacts(outDir: string, schema: ThsSchema, config: NetlifyUploadBuildConfig) {
  const functionsDir = path.join(outDir, config.functionsDirectory);
  ensureDir(functionsDir);
  fs.writeFileSync(path.join(outDir, 'package.json'), renderNetlifyPackageJson(schema));
  fs.writeFileSync(path.join(outDir, 'netlify.toml'), renderNetlifyToml(config));
  fs.writeFileSync(path.join(outDir, 'NETLIFY-UPLOADS.md'), renderNetlifyReadme(config));
  fs.writeFileSync(path.join(functionsDir, '_tokenhost-upload-shared.mjs'), renderNetlifyUploadShared(config));
  fs.writeFileSync(path.join(functionsDir, `${config.startFunctionName}.mjs`), renderNetlifyUploadStart(config));
  fs.writeFileSync(path.join(functionsDir, `${config.statusFunctionName}.mjs`), renderNetlifyUploadStatus());
  fs.writeFileSync(path.join(functionsDir, `${config.workerFunctionName}.mjs`), renderNetlifyUploadWorker());
}
...
function buildFromSchema(
  ...
  if (netlifyUploadBuild) {
    writeNetlifyUploadArtifacts(resolvedOutDir, schema, netlifyUploadBuild);
  }
  ...
  if (!opts.quiet) {
    ...
    if (netlifyUploadBuild) {
      console.log(`Wrote netlify.toml + Netlify upload functions`);
    }
