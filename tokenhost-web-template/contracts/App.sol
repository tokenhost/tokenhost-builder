...
contract App {
        ...
        returns (address)
    {
        ...
        if (!user_map[tx.origin].exists) {
            user_map[tx.origin] = create_user_on_new_Txt(mynew);
        }
        user_map[tx.origin].Txt_list.push(mynew);
        user_map[tx.origin].Txt_list_length += 1;

        Txt_list.push(mynew);
        // The length of Txt_list is tracked by the array length

        emit NewTxt(tx.origin, mynew);
    ...
    function new_Hashtag(string memory tag) public returns (address) {
        ...
        if (!user_map[tx.origin].exists) {
            user_map[tx.origin] = create_user_on_new_Hashtag(mynew);
        }
        user_map[tx.origin].Hashtag_list.push(mynew);
        user_map[tx.origin].Hashtag_list_length += 1;

        Hashtag_list.push(mynew);
        // The length of Hashtag_list is tracked by the array length

        emit NewHashtag(tx.origin, mynew);
